...
function GlitchMap(){
	...
	********************************************************/	
	...
	//FIX: Added in the 'map' input argument
	function drawBlock(index,grid,div,map){
		...
		if(index < current_data.tileset_blocks.length){
			for(var i=0;i<16;i++){
				...
				//FIX: to click through the tile to the div so the mouseup event still fires
				if( map ) tile.style.pointerEvents='none';
	...
	function replaceBlockTile(index){
		...
		for(var i=0;i<blocks.length;i++){
			...
			//FIX: Instead of replacing the tiles, just move the background to the new position
			...
			//	blocks[i].removeChild(replace);
				...
				var t = blocks[i].childNodes[index];
				t.setAttribute('index',current_tile);
				drawTileBg(t);
			}
		}
	...
	***********************************************************/
	...
	function mapBlockClick(el){
		...
		if(currentTab == "map"){
			...
			//FIX: added 'true' to the input 
			drawBlock(currentBlock,map_borders,block,true);
			addTilesMouseOverOut(block);
