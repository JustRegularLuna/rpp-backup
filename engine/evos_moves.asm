...
.done
	...
	ld [hl], c
	call PrepareRelearnableBaseMoves  ;;;;;; TODO: Delete PrepareRelearnableBaseMoves after base moves have been moved out of BaseStats
	ret

PrepareRelearnableBaseMoves:
	ld a, [wWhichPokemon]
	ld c, a
	ld b, 0
	ld hl, wPartySpecies
	...
	ld c, a ; c = count of relearnable moves
.loop
	ld a, [hli]
	and a
	ret z
...
.relearnableMove
	pop de
	push hl
	; Check if move is already in the list
	ld hl, wRelearnableMoves + 1
.moveExistsLoop
	ld a, [hli]
	cp $ff
	jr z, .moveDoesNotExist
	cp b
	jr z, .moveExists
	jr .moveExistsLoop
.moveExists
...
.moveDoesNotExist
	...
	push de
	ld de, wRelearnableMoves + 1
	ld hl, wRelearnableMoves
	inc [hl]
.insertLoop
	; b = move to insert
	ld a, [de]
	ld c, a  ; c = move to shift right
	ld a, b
	ld [de], a
	inc de
	ld a, c
